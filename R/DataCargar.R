...
# Define R6 class
#' @export
DataCargar <- R6::R6Class("DataCargar",
  public = list(
    datos = NULL,
    # Initialize db type: "mysql", "sqlite", "zip", "tar", "db" etc or path to folder
    ...
    leer = function(table_name, ...) {
      ...
      if (all(table_name %in% self.list())){
        if (self.datos %in% c("zip", "tar", "dir")){
          ...
          y <- files[table_name]
          leer.file(y, ...)
        }else if (self.datos %in% c("sqlite", "db", "sqlite3", "db3", "mysql", "sql")){
    ...
    #' @return The DataCargar object itself (invisibly), allowing for method chaining
    #' @export
    info = function(){
      ...
      tryCatch(
        {
          ...
          con <- do.call(DBI::dbConnect, c(list(RMariaDB::MariaDB()), args, list(password = passwd())))
